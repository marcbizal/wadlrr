...
module.exports = {
   ...
   * @returns {Promise} - resolves to the index that the file was found at
   */
   ...
   * @param {Integer} fd - file descriptor for the open WAD file
   * @param {Object} object - the object's metadata
   * @returns {Promise} - resolves to a buffer containing the object data
   */

  getDataForObject(fd, object) {
  ...
  getObject(wadMeta, object) {
    return fsp.open(wadMeta.source, 'r')
      .then(fd => Promise.all([fd, this.getDataForObject(fd, object)]))
      .then(([fd, buffer]) => {
        fsp.close(fd);
        return buffer;
      });
      ...
      fsp.mkdirs(outputPath).then(() => fsp.open(wadMeta.source, 'r'))
        .then((fd) => {
          wadMeta.objects.forEach((object, index) => {
            this.getDataForObject(fd, object)
            .then(buffer => Promise.all([
              buffer,
